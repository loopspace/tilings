...
This is now the \Verb+penrose+ TikZ library, meaning that old documents that use the \Verb+penrose+ TikZ library should still work.
New documents shouldn't be written using:
...
The \Verb!<sides>! is a list of sides for matching rules and positioning.
The first side in the list corresponds to the edge between the first two vertices, through to the last side which corresponds to the edge between the last and first vertices.
...
\tikzset{
  every tile/.style={draw,ultra thick},
...
\item Super Cluster H \tikz[baseline=-.5ex] \pic[scale=.5,draw,super cluster H];
\item Super Cluster F \tikz[baseline=-.5ex] \pic[scale=.5,draw,super cluster F];
\item Super Cluster T \tikz[baseline=-.5ex] \pic[scale=.5,draw,super cluster T];
\item Super Cluster P \tikz[baseline=-.5ex] \pic[scale=.5,draw,super cluster P];
...
The \(P\) and \(F\) tiles have no area (so are not clipped by default) and the \(H\) and \(T\) tiles have edge deformations applied by default so that they have the appearance of the aperiodical hat tile.
...
Each of the tiles comes with a shorthand key which specifies the \Verb+pic type+ and also invokes the key \Verb+every tile+.
That is, the key \Verb+dart+ calls the \Verb+every tile+ key and specifies the \Verb+pic type+ as \Verb+dart+.
...
If the shortcut keys are used to specify the tiles, this can be put in the \Verb+every tile+ style.
...
\item \Verb+every <name> clip+ where \Verb+name+ is the tile name (not the pic name).
\item \Verb+this tile clip+.
\end{enumerate}

\item A set of keys are used to allow arbitrary code to be executes before the tile path is rendered.
These are:
...
This library contains an implementation of this for each of the Penrose tilings.
(The intention is to implement this for the super clusters for the aperiodical polykite tilings as well).
...
The various tiles are built from labelled paths together with their reverses.
